...
-- | The monad in which all GPU computations are done. 'Shader s a' lives in an object space 'os' and a context with format 'f', closing over an environent of type 's'.
newtype Shader s a = Shader (ShaderM s a)  deriving (MonadPlus, Monad, Alternative, Applicative, Functor)

-- | Map the environment to a different environment and run a Shader in that sub environment, returning it's result.
mapShader :: (s -> s') -> Shader s' a -> Shader s a
...
maybeShader :: (s -> Maybe s') -> Shader s' () -> Shader s ()
...
chooseShader :: (s -> Either s' s'') -> Shader s' a -> Shader s'' a -> Shader s a
...
silenceShader :: Shader s a -> Shader s a
...
type CompiledShader s = s -> Render ()
...
compileShader :: (ContextHandler ctx, MonadIO m, MonadException m) => Shader x () -> ContextT ctx m (CompiledShader x)
