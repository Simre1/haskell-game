...
data Texture1D a = Texture1D TexName Size1 MaxLevels
data Texture1DArray a = Texture1DArray TexName Size2 MaxLevels
data Texture2D a = Texture2D TexName Size2 MaxLevels
                    | RenderBuffer2D TexName Size2
data Texture2DArray a = Texture2DArray TexName Size3 MaxLevels
data Texture3D a = Texture3D TexName Size3 MaxLevels
data TextureCube a = TextureCube TexName Size1 MaxLevels
...
newTexture1D :: forall ctx w f c m. (ContextHandler ctx, ColorSampleable c, MonadIO m) => Format c -> Size1 -> MaxLevels -> ContextT ctx m (Texture1D (Format c))
newTexture1DArray :: forall ctx w f c m. (ContextHandler ctx, ColorSampleable c, MonadIO m) => Format c -> Size2 -> MaxLevels -> ContextT ctx m (Texture1DArray (Format c))
newTexture2D :: forall ctx w f c m. (ContextHandler ctx, TextureFormat c, MonadIO m) => Format c -> Size2 -> MaxLevels -> ContextT ctx m (Texture2D (Format c))
newTexture2DArray :: forall ctx w f c m. (ContextHandler ctx, ColorSampleable c, MonadIO m) => Format c -> Size3 -> MaxLevels -> ContextT ctx m (Texture2DArray (Format c))
newTexture3D :: forall ctx w f c m. (ContextHandler ctx, ColorRenderable c, MonadIO m) => Format c -> Size3 -> MaxLevels -> ContextT ctx m (Texture3D (Format c))
newTextureCube :: forall ctx w f c m. (ContextHandler ctx, ColorSampleable c, MonadIO m) => Format c -> Size1 -> MaxLevels -> ContextT ctx m (TextureCube (Format c))
...
getGlValue :: (ContextHandler ctx, MonadIO m) =>  GLenum -> ContextT ctx m Int
...
texture1DLevels :: Texture1D f -> Int
texture1DArrayLevels :: Texture1DArray f -> Int
texture2DLevels :: Texture2D f -> Int
texture2DArrayLevels :: Texture2DArray f -> Int
texture3DLevels :: Texture3D f -> Int
textureCubeLevels :: TextureCube f -> Int
...
texture1DSizes :: Texture1D f -> [Size1]
texture1DArraySizes :: Texture1DArray f -> [Size2]
texture2DSizes :: Texture2D f -> [Size2]
texture2DArraySizes :: Texture2DArray f -> [Size3]
texture3DSizes :: Texture3D f -> [Size3]
textureCubeSizes :: TextureCube f -> [Size1]
...
makeTex :: (ContextHandler ctx, MonadIO m) => ContextT ctx m TexName
makeTex = do
...
makeRenderBuff :: (ContextHandler ctx, MonadIO m) => ContextT ctx m TexName
makeRenderBuff = do
...
writeTexture2DFromBuffer     :: forall ctx b c h w f m. (ContextHandler ctx, MonadIO m, BufferFormat b, ColorSampleable c, BufferColor (Color c (ColorElement c)) (HostFormat b) ~ b) => Texture2D (Format c) -> Level -> StartPos2 -> Size2 -> Buffer b -> BufferStartPos -> ContextT ctx m ()
writeTexture2DArrayFromBuffer:: forall ctx b c h w f m. (ContextHandler ctx, MonadIO m, BufferFormat b, ColorSampleable c, BufferColor (Color c (ColorElement c)) (HostFormat b) ~ b) => Texture2DArray (Format c) -> Level -> StartPos3 -> Size3 -> Buffer b -> BufferStartPos -> ContextT ctx m ()
writeTexture3DFromBuffer     :: forall ctx b c h w f m. (ContextHandler ctx, MonadIO m, BufferFormat b, ColorSampleable c, BufferColor (Color c (ColorElement c)) (HostFormat b) ~ b) => Texture3D (Format c) -> Level -> StartPos3 -> Size3 -> Buffer b -> BufferStartPos -> ContextT ctx m ()
writeTextureCubeFromBuffer   :: forall ctx b c h w f m. (ContextHandler ctx, MonadIO m, BufferFormat b, ColorSampleable c, BufferColor (Color c (ColorElement c)) (HostFormat b) ~ b) => TextureCube (Format c) -> Level -> CubeSide -> StartPos2 -> Size2 -> Buffer b -> BufferStartPos -> ContextT ctx m ()


readTexture1D      :: forall ctx a b c h w f m. (ContextHandler ctx, MonadAsyncException m, MonadIO m, BufferFormat b, ColorSampleable c, BufferColor (Color c (ColorElement c)) h ~ b, h ~ HostFormat b) => Texture1D (Format c) -> Level -> StartPos1 -> Size1 -> (a -> h -> ContextT ctx m a) -> a -> ContextT ctx m a
readTexture1DArray :: forall ctx a b c h w f m. (ContextHandler ctx, MonadAsyncException m, MonadIO m, BufferFormat b, ColorSampleable c, BufferColor (Color c (ColorElement c)) h ~ b, h ~ HostFormat b) => Texture1DArray (Format c) -> Level -> StartPos2 -> Size1 -> (a -> h -> ContextT ctx m a) -> a -> ContextT ctx m a
readTexture2D      :: forall ctx a b c h w f m. (ContextHandler ctx, MonadAsyncException m, MonadIO m, BufferFormat b, ColorSampleable c, BufferColor (Color c (ColorElement c)) h ~ b, h ~ HostFormat b) => Texture2D (Format c) -> Level -> StartPos2 -> Size2 -> (a -> h -> ContextT ctx m a) -> a -> ContextT ctx m a
readTexture2DArray :: forall ctx a b c h w f m. (ContextHandler ctx, MonadAsyncException m, MonadIO m, BufferFormat b, ColorSampleable c, BufferColor (Color c (ColorElement c)) h ~ b, h ~ HostFormat b) => Texture2DArray (Format c) -> Level -> StartPos3 -> Size2 -> (a -> h -> ContextT ctx m a) -> a -> ContextT ctx m a
readTexture3D      :: forall ctx a b c h w f m. (ContextHandler ctx, MonadAsyncException m, MonadIO m, BufferFormat b, ColorSampleable c, BufferColor (Color c (ColorElement c)) h ~ b, h ~ HostFormat b) => Texture3D (Format c) -> Level -> StartPos3 -> Size2 -> (a -> h -> ContextT ctx m a) -> a -> ContextT ctx m a
readTextureCube    :: forall ctx a b c h w f m. (ContextHandler ctx, MonadAsyncException m, MonadIO m, BufferFormat b, ColorSampleable c, BufferColor (Color c (ColorElement c)) h ~ b, h ~ HostFormat b) => TextureCube (Format c) -> Level -> CubeSide -> StartPos2 -> Size2 -> (a -> h -> ContextT ctx m a) -> a -> ContextT ctx m a

readTexture1DToBuffer     :: forall ctx b c w f m. (ContextHandler ctx, MonadIO m, BufferFormat b, ColorSampleable c, BufferColor (Color c (ColorElement c)) (HostFormat b) ~ b) => Texture1D (Format c) -> Level -> StartPos1 -> Size1 -> Buffer b -> BufferStartPos -> ContextT ctx m ()
readTexture1DArrayToBuffer:: forall ctx b c w f m. (ContextHandler ctx, MonadIO m, BufferFormat b, ColorSampleable c, BufferColor (Color c (ColorElement c)) (HostFormat b) ~ b) => Texture1DArray (Format c) -> Level -> StartPos2 -> Size1 -> Buffer b -> BufferStartPos -> ContextT ctx m ()
readTexture2DToBuffer     :: forall ctx b c w f m. (ContextHandler ctx, MonadIO m, BufferFormat b, ColorSampleable c, BufferColor (Color c (ColorElement c)) (HostFormat b) ~ b) => Texture2D (Format c) -> Level -> StartPos2 -> Size2 -> Buffer b -> BufferStartPos -> ContextT ctx m ()
readTexture2DArrayToBuffer:: forall ctx b c w f m. (ContextHandler ctx, MonadIO m, BufferFormat b, ColorSampleable c, BufferColor (Color c (ColorElement c)) (HostFormat b) ~ b) => Texture2DArray (Format c) -> Level -> StartPos3 -> Size2 -> Buffer b -> BufferStartPos -> ContextT ctx m ()
readTexture3DToBuffer     :: forall ctx b c w f m. (ContextHandler ctx, MonadIO m, BufferFormat b, ColorSampleable c, BufferColor (Color c (ColorElement c)) (HostFormat b) ~ b) => Texture3D (Format c) -> Level -> StartPos3 -> Size2 -> Buffer b -> BufferStartPos -> ContextT ctx m ()
readTextureCubeToBuffer   :: forall ctx b c w f m. (ContextHandler ctx, MonadIO m, BufferFormat b, ColorSampleable c, BufferColor (Color c (ColorElement c)) (HostFormat b) ~ b) => TextureCube (Format c) -> Level -> CubeSide -> StartPos2 -> Size2 -> Buffer b-> BufferStartPos -> ContextT ctx m ()
...
generateTexture1DMipmap :: (ContextHandler ctx, MonadIO m) => Texture1D f -> ContextT ctx m ()
generateTexture1DArrayMipmap :: (ContextHandler ctx, MonadIO m) => Texture1DArray f -> ContextT ctx m ()
generateTexture2DMipmap :: (ContextHandler ctx, MonadIO m) => Texture2D f -> ContextT ctx m ()
generateTexture2DArrayMipmap :: (ContextHandler ctx, MonadIO m) => Texture2DArray f -> ContextT ctx m ()
generateTexture3DMipmap :: (ContextHandler ctx, MonadIO m) => Texture3D f -> ContextT ctx m ()
generateTextureCubeMipmap :: (ContextHandler ctx, MonadIO m) => TextureCube f -> ContextT ctx m ()
...
newSampler1D :: forall s c. ColorSampleable c => (s -> (Texture1D (Format c), SamplerFilter c, (EdgeMode,  BorderColor c))) -> Shader s (Sampler1D (Format c))
newSampler1DArray :: forall s c. ColorSampleable c => (s -> (Texture1DArray (Format c), SamplerFilter c, (EdgeMode, BorderColor c))) -> Shader s (Sampler1DArray (Format c))
newSampler2D :: forall s c. ColorSampleable c => (s -> (Texture2D (Format c), SamplerFilter c, (EdgeMode2, BorderColor c))) -> Shader s (Sampler2D (Format c))
newSampler2DArray :: forall s c. ColorSampleable c => (s -> (Texture2DArray (Format c), SamplerFilter c, (EdgeMode2, BorderColor c))) -> Shader s (Sampler2DArray (Format c))
newSampler3D :: forall s c. ColorRenderable c => (s -> (Texture3D (Format c), SamplerFilter c, (EdgeMode3, BorderColor c))) -> Shader s (Sampler3D (Format c))
newSamplerCube :: forall s c. ColorSampleable c => (s -> (TextureCube (Format c), SamplerFilter c)) -> Shader s (SamplerCube (Format c))

newSampler1DShadow :: forall s d. DepthRenderable d => (s -> (Texture1D (Format d), SamplerFilter d, (EdgeMode, BorderColor d), ComparisonFunction)) -> Shader s (Sampler1D Shadow)
newSampler1DArrayShadow :: forall s d. DepthRenderable d => (s -> (Texture1DArray (Format d), SamplerFilter d, (EdgeMode, BorderColor d), ComparisonFunction)) -> Shader s (Sampler1DArray Shadow)
newSampler2DShadow :: forall s d. DepthRenderable d => (s -> (Texture2D d, SamplerFilter (Format d), (EdgeMode2, BorderColor d), ComparisonFunction)) -> Shader s (Sampler2D Shadow)
newSampler2DArrayShadow :: forall s d. DepthRenderable d => (s -> (Texture2DArray (Format d), SamplerFilter d, (EdgeMode2, BorderColor d), ComparisonFunction)) -> Shader s (Sampler2DArray Shadow)
newSamplerCubeShadow :: forall s d. DepthRenderable d => (s -> (TextureCube (Format d), SamplerFilter d, ComparisonFunction)) -> Shader s (SamplerCube Shadow)
...
getTexture1DImage :: Texture1D f -> Level -> Render (Image f)
getTexture1DArrayImage :: Texture1DArray f -> Level -> Int -> Render (Image f)
getTexture2DImage :: Texture2D f -> Level -> Render (Image f)
getTexture2DArrayImage :: Texture2DArray f -> Level -> Int -> Render (Image f)
getTexture3DImage :: Texture3D f -> Level -> Int -> Render (Image f)
getTextureCubeImage :: TextureCube f -> Level -> CubeSide -> Render (Image f)
