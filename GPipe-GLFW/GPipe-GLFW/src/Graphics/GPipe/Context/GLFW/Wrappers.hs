...
withWindow :: MonadIO m => (GLFW.Window -> IO a) -> GPipe.Window c ds -> GPipe.ContextT Handler.Handle m (Maybe a)
...
withWindowRPC :: MonadIO m => (Call.OnMain a -> GLFW.Window -> IO a) -> GPipe.Window c ds -> GPipe.ContextT Handler.Handle m (Maybe a)
...
wrapWindowFun :: MonadIO m => (Call.OnMain b -> GLFW.Window -> a -> IO b) -> GPipe.Window c ds -> a -> GPipe.ContextT Handler.Handle m (Maybe b)
...
wrapCallbackSetter :: (MonadIO m, Functor g) => (Call.OnMain a -> GLFW.Window -> g (GLFW.Window -> b) -> IO a) -> GPipe.Window c ds -> g b -> GPipe.ContextT Handler.Handle m (Maybe a)
