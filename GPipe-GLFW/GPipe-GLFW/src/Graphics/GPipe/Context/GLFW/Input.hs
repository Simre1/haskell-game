...
setKeyCallback :: MonadIO m => GPipe.Window c ds -> Maybe (Key -> Int -> KeyState -> ModifierKeys -> IO ()) -> GPipe.ContextT Handle m (Maybe ())
setKeyCallback = wrapCallbackSetter Call.setKeyCallback

-- | Poll for the 'KeyState' of a 'Key'.
getKey :: MonadIO m => GPipe.Window c ds -> Key -> GPipe.ContextT Handle m (Maybe KeyState)
...
setStickyKeysInputMode :: MonadIO m => GPipe.Window c ds -> StickyKeysInputMode -> GPipe.ContextT Handle m (Maybe ())
setStickyKeysInputMode = wrapWindowFun Call.setStickyKeysInputMode

getStickyKeysInputMode :: MonadIO m => GPipe.Window c ds -> GPipe.ContextT Handle m (Maybe StickyKeysInputMode)
...
setCharCallback :: MonadIO m => GPipe.Window c ds -> Maybe (Char -> IO ()) -> GPipe.ContextT Handle m (Maybe ())
...
setCursorPosCallback :: MonadIO m => GPipe.Window c ds -> Maybe (Double -> Double -> IO ()) -> GPipe.ContextT Handle m (Maybe ())
setCursorPosCallback = wrapCallbackSetter Call.setCursorPosCallback

-- | Poll for the location of the mouse.
getCursorPos :: MonadIO m => GPipe.Window c ds -> GPipe.ContextT Handle m (Maybe (Double, Double))
...
setCursorInputMode :: MonadIO m => GPipe.Window c ds -> CursorInputMode -> GPipe.ContextT Handle m (Maybe ())
setCursorInputMode = wrapWindowFun Call.setCursorInputMode

getCursorInputMode :: MonadIO m => GPipe.Window c ds -> GPipe.ContextT Handle m (Maybe CursorInputMode)
...
setCursor :: MonadIO m => GPipe.Window c ds -> Cursor -> GPipe.ContextT Handle m (Maybe ())
...
setCursorEnterCallback :: MonadIO m => GPipe.Window c ds -> Maybe (CursorState -> IO ()) -> GPipe.ContextT Handle m (Maybe ())
...
setMouseButtonCallback :: MonadIO m => GPipe.Window c ds -> Maybe (MouseButton -> MouseButtonState -> ModifierKeys -> IO ()) -> GPipe.ContextT Handle m (Maybe ())
setMouseButtonCallback = wrapCallbackSetter Call.setMouseButtonCallback

-- | Poll for the 'MouseButtonState' of a 'MouseButton'.
getMouseButton :: MonadIO m => GPipe.Window c ds -> MouseButton -> GPipe.ContextT Handle m (Maybe MouseButtonState)
...
setStickyMouseButtonsInputMode :: MonadIO m => GPipe.Window c ds -> StickyMouseButtonsInputMode -> GPipe.ContextT Handle m (Maybe ())
setStickyMouseButtonsInputMode = wrapWindowFun Call.setStickyMouseButtonsInputMode

getStickyMouseButtonsInputMode :: MonadIO m => GPipe.Window c ds -> GPipe.ContextT Handle m (Maybe StickyMouseButtonsInputMode)
...
setScrollCallback :: MonadIO m => GPipe.Window c ds -> Maybe (Double -> Double -> IO ()) -> GPipe.ContextT Handle m (Maybe ())
...
getClipboardString :: MonadIO m => GPipe.Window c ds -> GPipe.ContextT Handle m (Maybe (Maybe String))
getClipboardString = withWindowRPC Call.getClipboardString

-- | Store a UTF-8 encoded string in the system clipboard.
setClipboardString :: MonadIO m => GPipe.Window c ds -> String -> GPipe.ContextT Handle m (Maybe ())
...
setDropCallback :: MonadIO m => GPipe.Window c ds -> Maybe ([String] -> IO ()) -> GPipe.ContextT Handle m (Maybe ())
